...
public:
	...
	{
		memory::CallVFunc<void>(this, 3, iStartFrame, bValidFrame, nIncomingAcknowledged, nOutgoingCommand);
	}

	void GetLocalViewAngles(QAngle& angView)
	{
		memory::CallVFunc<void, QAngle&>(this, 12, angView);
	}

	void SetLocalViewAngles(QAngle& angView)
	{
		memory::CallVFunc<void, QAngle&>(this, 13, angView);
	}

	void CheckMovingGround(CBaseEntity* pEntity, double dbFrametime)
	{
		memory::CallVFunc<void>(this, 18, pEntity, dbFrametime);
	}

	void SetupMove(CBaseEntity* pEntity, CUserCmd* pCmd, IMoveHelper* pHelper, CMoveData* pMoveData)
	{
		memory::CallVFunc<void>(this, 20, pEntity, pCmd, pHelper, pMoveData);
	}

	void FinishMove(CBaseEntity* pEntity, CUserCmd* pCmd, CMoveData* pMoveData)
	{
		memory::CallVFunc<void>(this, 21, pEntity, pCmd, pMoveData);
	}
};
