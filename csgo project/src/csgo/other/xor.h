...
#define XorStr( s ) s
#else
#define XorStr_( s ) jm::make_xorstr([]() { return s; }, std::make_index_sequence<sizeof( s ) / sizeof( *s )>{}, std::make_index_sequence<jm::detail::_buffer_size<sizeof( s )>()>{})
...
namespace jm
{
	...
	class xor_string {
		...
		__forceinline void _crypt_256_single(const std::uint64_t* keys, std::uint64_t* storage) noexcept
		{
			_mm256_store_si256(
		...
		__forceinline void _crypt_256(const std::uint64_t* keys, std::index_sequence<Idxs...>) noexcept
		{
			(_crypt_256_single(keys + Idxs * 4, _storage + Idxs * 4), ...);
		}

		__forceinline void _crypt_128_single(const std::uint64_t* keys, std::uint64_t* storage) noexcept
		{
			_mm_store_si128(
		...
		template<std::size_t... Idxs>
		__forceinline void _crypt_128(const std::uint64_t* keys, std::index_sequence<Idxs...>) noexcept
		{
		...
		__forceinline constexpr void _copy() noexcept
		{
			...
			static_cast<void>(std::initializer_list<std::uint64_t>{
				(const_cast<std::uint64_t*>(_storage))[Keys::idx] =
	...
	public:
		...
		__forceinline void crypt() noexcept
		{
			...
			static_cast<void>(std::initializer_list<std::uint64_t>{ (const_cast<std::uint64_t*>(keys))[Keys::idx] = Keys::key... });
	...
	__forceinline constexpr auto make_xorstr(Tstr str_lambda, std::index_sequence<StringIndices...>, std::index_sequence<KeyIndices...>) noexcept
	{
		return xor_string<detail::tstring_<str_lambda()[StringIndices]...>, detail::_ki<KeyIndices, detail::key8<KeyIndices>()>...>{};
	}
}