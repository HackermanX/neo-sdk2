...
public:
	...
	void* operator new(std::size_t nAllocSize)
	{
		...
		return memory::CallVFunc<void*>(pKeyValuesSystem, 1, nAllocSize);
	}
	...
	void operator delete(void* pMemory)
	{
		...
		memory::CallVFunc<void>(pKeyValuesSystem, 2, pMemory);
	}
	...
	const char* GetName()
	{
		...
		return memory::CallVFunc<const char*>(pKeyValuesSystem, 4, *reinterpret_cast<std::uint8_t*>(reinterpret_cast<std::uintptr_t>(this) + 0x3) | (*reinterpret_cast<std::uint16_t*>(reinterpret_cast<std::uintptr_t>(this) + 0x12) << 8));
	}

	void Init(const char* szKeyName)
	{
		using InitKeyValuesFn = void(__thiscall*)(void*, const char*);
		static auto oInitKeyValues = reinterpret_cast<InitKeyValuesFn>(memory::PatternScan(CLIENT_DLL, XorStr("55 8B EC 51 33 C0 C7 45"))); // @xref: "OldParticleSystem_Destroy"
	...
	void LoadFromBuffer(char const* szResourceName, const char* szBuffer, void* szFileSystem = nullptr, const char* szPathID = nullptr, void* pfnEvaluateSymbolProc = nullptr)
	{
		...
		static auto oLoadFromBuffer = reinterpret_cast<LoadFromBufferFn>(memory::PatternScan(CLIENT_DLL, XorStr("55 8B EC 83 E4 F8 83 EC 34 53 8B 5D 0C 89"))); // @xref: "KeyValues::LoadFromBuffer(%s%s%s): Begin"
	...
	CKeyValues* FindKey(const char* szKeyName, bool bCreate)
	{
		...
		static auto oFindKey = reinterpret_cast<FindKeyFn>(memory::PatternScan(CLIENT_DLL, XorStr("55 8B EC 83 EC 1C 53 8B D9 85 DB")));
	...
	void SetString(const char* szKeyName, const char* szValue)
	{
		...
		static auto oSetString = reinterpret_cast<SetStringFn>(memory::PatternScan(CLIENT_DLL, XorStr("55 8B EC A1 ? ? ? ? 53 56 57 8B F9 8B 08 8B 01")));
